...
| 19 | [Hardware detection and enablement](phases/phase-19-hardware-enablement.md) | 1, 2, 6 | Claude + user | One hardware map (pci/usb/dmi/hda) applied by `symphony-hardware` at install, first login, update and on demand, plus a scan naming unhandled hardware; sound works on the Alienware; `power-profiles-daemon`; AlienFX takes the theme colour (bonus) | In progress |
