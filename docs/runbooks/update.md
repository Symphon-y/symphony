...
the checkout being worked on. One command, `autarchy-update`, in every install
since Phase 18 (D-0079). Verified by `tests/unit/autarchy-update.bats` and
`tests/acceptance/phase-18.bats`.

**What this is:** an installed machine has no repo on it. Its OS content is the
root-owned payload at `/usr/local/share/autarchy/current` (D-0067), and a release
is a new payload: `autarchy-<tag>-payload.tar.zst` on the GitHub Release for that
tag, signed with minisign (D-0078). `autarchy-update` downloads it, verifies the
signature against `/etc/autarchy/release.pub`, takes a Btrfs snapshot, swaps the
...
```sh
autarchy-update version     # the installed release, e.g. 2026.09.22 (or local-<sha> on the dev seat)
autarchy-update check       # the latest release, and a diff of packages/ and migrations/
```
...
2. Downloads the payload, its `.minisig` and `.sha256`; verifies the signature
   with `minisign -V -p /etc/autarchy/release.pub`, then the checksum. A
...
The payload's own `VERSION` is what `autarchy-update version` reports afterwards;
there is no separate marker to get out of sync.

## 3. Roll back

```sh
autarchy-update rollback
```
...
```sh
autarchy-update apply --from ~/Projects/Arch
```
