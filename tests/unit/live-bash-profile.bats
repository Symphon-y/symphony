...
# installer under cage -- unless the kernel command line says symphony.nogui, the way
...
setup() {
  ...
  PATH="$bin:$PATH"
  export SYMPHONY_CMDLINE_FILE="$BATS_TEST_TMPDIR/cmdline"
}
...
@test "tty1 starts the GUI installer under cage on a normal boot" {
  echo "BOOT_IMAGE=/arch/boot/x86_64/vmlinuz-linux archisobasedir=arch quiet" >"$SYMPHONY_CMDLINE_FILE"
  login
  run cat "$STUB_LOG"
  assert_output --partial "cage -- /root/symphony/gui/symphony-installer"
}

@test "symphony.nogui on the kernel command line skips the GUI and leaves a terminal" {
  echo "BOOT_IMAGE=/arch/boot/x86_64/vmlinuz-linux archisobasedir=arch symphony.nogui" >"$SYMPHONY_CMDLINE_FILE"
  login
  assert_success
  run cat "$STUB_LOG"
  assert_output ""
  ...
  run bash -c "source '$PROFILE'"
  assert_output --partial "symphony-install"
}

@test "only the exact word disables it, not a lookalike" {
  echo "symphony.nogui-not symphony.nogui.x" >"$SYMPHONY_CMDLINE_FILE"
  login
  run cat "$STUB_LOG"
...
@test "other terminals never start the GUI" {
  echo "quiet" >"$SYMPHONY_CMDLINE_FILE"
...
@test "the banner says how to get a terminal on tty1 for diagnosing" {
  run grep -F 'symphony.nogui' "$PROFILE"
  assert_success
}
