...
@test "prints the parameter of a quirk whose DMI pattern matches this machine" {
  printf 'dmi:*:svnAlienware:pnAlienware*14:*  module_blacklist=dell_rbtn  # bogus slider\n' >"$MAP"
  dmi "$ALIENWARE_14"
  run "$SCRIPT"
  assert_success
...
@test "prints nothing for another model of the same vendor" {
  printf 'dmi:*:svnAlienware:pnAlienware*14:*  module_blacklist=dell_rbtn\n' >"$MAP"
  dmi "$ALIENWARE_17"
  run "$SCRIPT"
  assert_success
  assert_output ""
...
@test "prints every matching parameter, sorted, each once" {
  {
    printf 'dmi:*:svnAlienware:*  zzz_last\n'
    printf 'dmi:*:svnAlienware:pnAlienware*14:*  aaa_first=1\n'
    printf 'dmi:*:cvnAlienware:*  zzz_last\n'
    printf 'dmi:*:svnSomeoneElse:*  never\n'
  } >"$MAP"
  dmi "$ALIENWARE_14"
  run "$SCRIPT"
  assert_success
...
@test "comments and blank lines are ignored" {
  printf '# a header\n\n   \ndmi:*:svnAlienware:*  quiet   # trailing note\n' >"$MAP"
  dmi "$ALIENWARE_14"
  run "$SCRIPT"
  assert_success
  assert_output "quiet"
...
@test "fails closed on a line with an extra field" {
  printf 'dmi:*:svnAlienware:*  quiet  loglevel=3\n' >"$MAP"
  dmi "$ALIENWARE_14"
  run "$SCRIPT"
  assert_failure 1
}
...
@test "fails closed on a parameter that is not a plain kernel parameter (it ends up on the boot command line)" {
  printf 'dmi:*:svnAlienware:*  quiet;reboot\n' >"$MAP"
  dmi "$ALIENWARE_14"
  run "$SCRIPT"
  assert_failure 1
}
