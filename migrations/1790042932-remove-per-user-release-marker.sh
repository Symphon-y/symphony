#!/usr/bin/env bash
# Removes ~/.local/state/symphony/current-release, the per-user marker the
...
# (/usr/local/share/symphony/current/VERSION, D-0079), so the marker is stale
# state that would only ever disagree. Idempotent: a no-op once gone.
set -euo pipefail

rm -f "$HOME/.local/state/symphony/current-release"
